...
export class AuthController {
    ...
    async login(
        ...
        switch (role) {
            case 'student':
                try {
                    const loginStudent = await this.studentService.findByEmail(payload.email);
                    const limitedData = StudentResponseDto.fromStudent(loginStudent);
                    const token = this.authService.generateJwtToken(loginStudent);
                    return response.status(HttpStatus.OK).json({ student: limitedData, token });
                } catch (error) {
            ...
            case 'staff':
                try {
                    ...
                    return response.status(HttpStatus.OK).json({ staff: limitedData, token });
                } catch (error) {
                    return response.status(error.status).json({ message: error.message });
                }
            default:
