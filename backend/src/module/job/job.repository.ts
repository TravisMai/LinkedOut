...
export class JobRepository extends Repository<Job> {
    ...
    async findJobsWithCriteria(criteria: Partial<Job>): Promise<Job[]> {
        ...
        if (criteria.workType) {
            query.andWhere('job.workType = :workType', { workType: criteria.workType });
        }

        if (criteria.company) {
            query.innerJoin('job.company', 'company')
                .andWhere('company.name LIKE :companyName', { companyName: `%${criteria.company}%` });
        }
