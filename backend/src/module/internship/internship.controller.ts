...
export class InternshipController {
    ...
    async applyInternship(@Req() req: Request, @Res() response: Response, @Param('id') id: string): Promise<Response> {
        try {
    ...
    @Get(':id')
    @UseGuards(JwtGuard)
    async findOne(@Res() response: Response, @Param('id') id: string): Promise<Response> {
        try {
            const internship = await this.internshipService.findOne(id);
            if (!internship) {
                return response.status(HttpStatus.NOT_FOUND).json({ message: 'Internship not found!' });
            }
    ...
    @AllowRoles(['staff'])
    @UseGuards(JwtGuard, RolesGuard)
    async update(@Res() response: Response, @Param('id') id: string, @Body() internship: Internship): Promise<Response> {
        try {
            const updatedInternship = await this.internshipService.update(id, internship);
