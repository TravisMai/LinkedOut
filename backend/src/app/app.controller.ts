...
import { Controller, Get, Post, Body, Param, Put, Res, HttpStatus, UseGuards, Req, Query } from '@nestjs/common';
...
export class AppController {
    ...
    @Get()
    @UseGuards(JwtGuard)
    async search(@Req() req: Request, @Res() response: Response, @Query('search') search: string): Promise<Response> {
        try {
            ...
            return response.status(HttpStatus.NOT_FOUND).json({ message: 'No result found!' });
        } catch (error) {
