...
export default function InternshipRecruitment() {
  ...
  return (
    <>
      <div className="mt-8 w-full mx-auto">
        <Typography variant="h5" className="text-center">Internship recruitment results</Typography>
                  ...
                  <TableCell align="center">
                    <Box sx={{ "& > :not(style)": { m: 0.1 } }}>
                      <Tooltip title="Internship Program">
                        {row?.document?.find(doc => doc.name === "Recruiting result") ? (
                          <IconButton
                            href={
                              // Get the link of the document named "Recuirtment result"
                              row?.document?.find(doc => doc.name === "Recruiting result")?.url ?? ""
                            }
                          >
                            <Download />
                          </IconButton>) : (
                          <Typography variant="body2" color="error">No recruitment result</Typography>
                        )}
                      </Tooltip>
                    </Box>
                  </TableCell>
