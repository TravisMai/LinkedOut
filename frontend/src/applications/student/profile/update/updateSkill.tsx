...
import { Delete, Facebook, GitHub, Google, LinkedIn, Star, Twitter } from '@mui/icons-material';
...
export default function UpdateSKill({ onClose }: { onClose: () => void }) {
    const handleClose = () => {
        onClose();
    };
    ...
    const [formData, setFormData] = useState<{ skill: skillType[] }>({
        skill: [],
    });
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        updatedFormData.skill[idx][key as keyof skillType] = value;
        setFormData(updatedFormData);
    };
                    ...
                    >
                        ...
                        <Box component="form" onSubmit={handleSubmit} sx={{ mx: 2, mt: 5, mb: 2 }}>
                            <Grid container spacing={2} justifyContent="center">
                                {formData.skill?.map((item: skillType, index) => (
                                    <Grid container direction='row' spacing={1} className='mt-3 mx-5 mb-6'>
                                        ...
                                        <DividerWithText className='mt-5' text="" muiElementIcon={<Star />} />
